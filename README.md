...
<body class="text-gray-800 custom-scrollbar">
    ...
    <script>
        ...
        const initialData = {
            settings: { salaryPerStudent: 50, pointReward: 5, excellentBonus: 10 }, 
            users: [
        ...
        const App = {
            ...
            loadDB() {
                try {
                    ...
                    if(!this.db.finance) this.db.finance = { budget: 0, transactions: [] };
            ...
            renderSidebar() {
                ...
                if (this.currentUser.role === 'admin') {
                    ...
                    const items = [
                        ...
                        { id: 'admin-finance', label: 'المالية والرواتب', icon: 'fa-coins' }
                    ];
            ...
            renderAdminDashboard() {
                ...
                const reportsToday = this.db.dailyLogs.filter(l => l.date === todayStr).length;
                ...
                area.innerHTML = `
                    ...
                    <div class="mb-8 ${criticalStudents.length === 0 ? 'hidden' : ''}">
                        <div class="bg-red-50 border border-red-100 p-4 rounded-3xl flex items-center justify-between">
                            <div class="flex items-center gap-4">
                                ...
                                <div>
                                    <h4 class="text-sm font-black text-red-800">تنبيه غياب متكرر</h4>
                                    <p class="text-[10px] text-red-600 font-bold">هناك ${criticalStudents.length} طلاب غائبون لـ 3 أيام متتالية.</p>
                                </div>
                            </div>
                            <button onclick="App.navigate('admin-students')" class="bg-red-600 text-white px-4 py-2 rounded-xl text-[10px] font-black">متابعة</button>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
                        <div class="lg:col-span-2">
                            <div class="bg-white rounded-3xl shadow-sm p-6 border border-gray-100">
                                <h3 class="text-lg font-black text-gray-800 mb-6 flex items-center"><i class="fas fa-chart-line ml-3 text-primary"></i> أداء الحلقات اليوم (${todayStr})</h3>
                                <div class="overflow-x-auto">
                                    <table class="w-full text-right">
                                        <thead>
                                            <tr class="text-gray-400 text-[10px] uppercase font-black border-b">
                                                <th class="pb-3">الحلقة</th>
                                                <th class="pb-3">المحفظ</th>
                                                <th class="pb-3 text-center">الطلاب</th>
                                                <th class="pb-3 text-center">حالة التقرير</th>
                                            </tr>
                                        </thead>
                                        <tbody class="divide-y divide-gray-50">
                                            ${this.db.circles.length > 0 ? this.db.circles.map(c => {
                                                const hasReport = this.db.dailyLogs.some(l => {
                                                    const st = this.db.students.find(s => s.id == l.studentId);
                                                    return st && st.circleId == c.id && l.date == todayStr;
                                                });
                                                return `
            ...
            deleteTransaction(id) {
                if(confirm('هل أنت متأكد من حذف هذه العملية المالية؟')) {
                    this.db.finance.transactions = this.db.finance.transactions.filter(t => t.id != id);
                    this.saveDB();
                    this.renderAdminFinance();
                    this.showToast("تم حذف العملية المالية");
                }
            },
                                        ...
                                        <tr class="hover:bg-blue-50/20 transition-all group ${isAbsent ? 'absent-row' : ''}">
                                            <td class="p-4">
                                                <p class="font-bold text-sm text-gray-800">
                                                    ${s.name}
                                                    ${isAbsent ? '<i class="fas fa-exclamation-triangle text-red-500 text-[10px] mr-1" title="تنبيه: غياب متكرر"></i>' : ''}
                                                </p>
                                            ...
                                            <td class="p-4 flex items-center space-x-3 space-x-reverse">
                                                <button onclick="App.openAddTestModal(${s.id})" class="text-orange-500 hover:scale-110" title="تسجيل اختبار"><i class="fas fa-medal"></i></button>
                                                <button onclick="App.viewStudentHistory(${s.id})" class="text-green-500 hover:scale-110" title="سجل المتابعة"><i class="fas fa-history"></i></button>
                                                <button onclick="App.openEditStudentModal(${s.id})" class="text-blue-400"><i class="fas fa-edit"></i></button>
                                                <button onclick="App.deleteStudent(${s.id})" class="text-red-400"><i class="fas fa-user-minus"></i></button>
                                            </td>
                                        </tr>
            ...
            renderTeacherDaily() {
                ...
                area.innerHTML = `
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6 no-print">
                        ...
                        <div class="bg-white p-6 rounded-3xl border border-gray-100 flex items-center justify-center">
                            <button onclick="App.markAllPresent()" class="text-secondary hover:text-green-700 font-black flex items-center gap-2">
                                <i class="fas fa-check-double text-xl"></i> تحضير الكل حاضر
                            </button>
                        </div>
                    </div>
                    <div class="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden page-animate">
                        <div class="p-6 border-b flex flex-col md:flex-row justify-between items-center bg-gray-50/50 gap-4">
                            ...
                            <div class="flex space-x-2 space-x-reverse">
                                <button onclick="window.print()" class="no-print bg-white border border-gray-100 text-gray-600 px-4 py-2 rounded-xl text-xs font-black">طباعة</button>
                            ...
                            <table class="w-full text-right">
                                <thead class="bg-gray-100 text-[10px] font-black text-gray-400 uppercase">
                                    <tr>
                                        ...
                                        <th class="p-4 text-center">الحفظ</th>
                                        ...
                                        return `
                                            <tr class="hover:bg-blue-50/10">
                                                <td class="p-4">
                                                    ...
                                                    <p class="text-[9px] text-gray-400">النقاط: ${s.points || 0}</p>
                                                </td>
                                                <td class="p-4">
                                                ...
                                                <td class="p-4 text-center no-print">
                                                  <button onclick="App.sendWhatsApp(${s.id})" class="text-green-500 hover:scale-110"><i class="fab fa-whatsapp text-lg"></i></button>
                                                </td>
                                            </tr>
                                        `;
                                    }).join('') : '<tr><td colspan="6" class="text-center py-10 text-gray-400">لا يوجد طلاب نشطين</td></tr>'}
                                </tbody>
                            </table>
                        </div>
                    </div>
            ...
            saveDailyLogs() {
                ...
                if(rows.length === 0) return this.showToast("لا يوجد بيانات", "error");
                ...
                rows.forEach(row => {
                    ...
                    const attValue = attEl.value;
                    const evalValue = row.querySelector('.log-exam').value;
                    ...
                    // منح النقاط تلقائياً في أول حفظ للسجل اليومي
                    if (idx === -1) {
                        const sIdx = this.db.students.findIndex(s => s.id == studentId);
                        if(sIdx > -1 && attValue === 'حاضر') {
                            let pts = this.db.settings.pointReward || 5;
                            if (evalValue.includes("ممتاز")) pts += (this.db.settings.excellentBonus || 10);
                            this.db.students[sIdx].points = (this.db.students[sIdx].points || 0) + pts;
                        }
                    }
                ...
                this.saveDB();
                this.showToast('تم حفظ التقارير ومنح نقاط التميز بنجاح', 'success');
                this.renderTeacherDaily(); // لتحديث النقاط الظاهرة
            },
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        ${students.length > 0 ? students.map(s => `
                            <div class="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm hover:border-primary transition-all">
                                ...
                                <div class="space-y-3">
                                    <div>
                                        <div class="flex justify-between text-[10px] mb-1"><span class="text-gray-400">الحفظ:</span><span class="font-black text-primary">${s.juzCount || 0} جزء</span></div>
                                    ...
                                    <div class="flex justify-between text-xs"><span class="text-gray-400">الحالة:</span><span class="font-bold ${s.status === 'نشط' ? 'text-green-600' : 'text-red-500'}">${s.status || 'نشط'}</span></div>
                                    <div class="grid grid-cols-2 gap-2 mt-4">
                                        <button onclick="App.viewStudentHistory(${s.id})" class="py-2 bg-gray-50 rounded-xl text-[10px] font-black hover:bg-primary hover:text-white transition-all">سجل المتابعة</button>
                        ...
                        `).join('') : '<p class="text-gray-400 text-center col-span-full py-10 font-bold">لا يوجد طلاب في حلقتك</p>'}
                    </div>
                `;
            },
            ...
            renderUserProfile() {
                ...
                area.innerHTML += `
                    <div class="mt-8 p-6 bg-orange-50 rounded-3xl border border-orange-100">
                        <h4 class="font-black text-orange-800 mb-2">تأمين الحساب</h4>
                        <p class="text-xs text-orange-600 mb-4">يُنصح بتغيير كلمة المرور دورياً للحفاظ على خصوصية البيانات.</p>
                        <button onclick="App.openChangePasswordModal()" class="bg-orange-500 text-white px-6 py-2 rounded-xl text-xs font-black shadow-lg">تغيير كلمة المرور</button>
                    </div>
                `;
            },
            ...
            renderAdminFinance() {
                ...
                area.innerHTML = `
                    <div class="bg-blue-600 text-white p-8 rounded-3xl mb-8 flex flex-col md:flex-row justify-between items-center shadow-xl">
                        <div>
                            ...
                            <div class="flex gap-2 mt-4">
                                ...
                                <button onclick="App.renderPayrollSection()" class="bg-white text-blue-600 px-4 py-2 rounded-xl text-xs font-bold transition-all"><i class="fas fa-hand-holding-usd ml-2"></i> عرض الرواتب</button>
                            </div>
                        </div>
                        <div class="mt-4 md:mt-0 bg-white/10 p-4 rounded-2xl">
                             <p class="text-[10px] font-black uppercase mb-1 opacity-70">إعدادات الراتب لكل طالب</p>
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        ...
                        <div class="lg:col-span-2 bg-white rounded-3xl border border-gray-100 overflow-hidden">
                            <div class="p-6 border-b bg-gray-50/50 font-black">آخر العمليات المالية</div>
                            <div class="max-h-96 overflow-y-auto">
                                <table class="w-full text-right text-sm">
                                    <thead class="bg-gray-50 sticky top-0">
                                        <tr><th class="p-4">البيان</th><th class="p-4">الفئة</th><th class="p-4 text-center">المبلغ</th><th class="p-4 text-center">حذف</th></tr>
                                    </thead>
                                    <tbody class="divide-y divide-gray-50">
                                        ...
                                        `).join('') : '<tr><td colspan="4" class="text-center py-10 text-gray-400 font-bold">لا يوجد عمليات</td></tr>'}
                                    </tbody>
                                </table>
                            </div>
                        </div>
                ...
                if(finForm) {
                    finForm.onsubmit = (e) => {
                        ...
                        if(amount <= 0 || isNaN(amount)) return alert('المبلغ غير صالح');
                ...
                payrollArea.innerHTML = `
                    <div class="bg-white p-6 rounded-3xl border-2 border-blue-100 page-animate">
                        <div class="flex justify-between items-center mb-4">
                            <h3 class="font-black text-blue-800"><i class="fas fa-calculator ml-2"></i> الرواتب المقدرة (نشطين فقط)</h3>
                            ...
                            ${this.db.users.filter(u => u.role === 'teacher').map(teacher => {
                                ...
                                return `
                                    <div class="bg-gray-50 p-4 rounded-2xl border border-gray-100">
                                        <p class="text-xs font-black text-gray-400">${teacher.name}</p>
            ...
            initChart() {
                ...
                this.myChart = new Chart(ctx, {
                    type: 'doughnut',
                    data: {
                        labels: ['متقدم (20+)', 'متوسط (5+)', 'مبتدئ'],
                        datasets: [{ data: [counts.advanced, counts.intermediate, counts.beginner], backgroundColor: ['#1a5fb4', '#2ec27e', '#e5a50a'], borderWidth: 0 }]
                    },
                    options: { cutout: '70%', responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 'bottom', labels: { font: { family: 'Cairo', weight: 'bold', size: 10 } } } } }
                });
            },
            ...
            setupStudentForm() {
                ...
                form.onsubmit = (e) => {
                    ...
                    this.showToast("تم حفظ الطالب", "success");
                    this.renderAdminStudents();
                    form.reset();
                };
            },
            ...
            viewStudentHistory(studentId) {
                ...
                const tests = (this.db.tests || []).filter(t => t.studentId == studentId);
                
                const area = document.getElementById('content-area');
                area.innerHTML = `
                    <div class="mb-6 flex items-center justify-between no-print">
                        <button onclick="App.navigate('${this.currentUser.role === 'admin' ? 'admin-students' : 'teacher-students'}')" class="text-xs font-black text-gray-500 hover:text-primary"><i class="fas fa-arrow-right ml-2"></i> العودة</button>
                        <h3 class="font-black text-xl">سجل الطالب: ${student.name}</h3>
                        <button onclick="window.print()" class="bg-primary text-white px-4 py-2 rounded-xl text-xs font-black">طباعة</button>
                    </div>
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                        ...
                        <div class="bg-white p-5 rounded-3xl border shadow-sm text-center">
                            <p class="text-[10px] text-gray-400 font-bold mb-1">النقاط</p>
                            <p class="text-xl font-black text-orange-500">${student.points || 0}</p>
                        </div>
                    </div>
                    <div class="bg-white rounded-3xl border border-gray-100 overflow-hidden shadow-sm">
                        <div class="p-4 bg-gray-50 font-black text-gray-800 text-sm">المتابعة اليومية</div>
                        <table class="w-full text-right text-xs">
                            <thead class="bg-gray-50 text-gray-400 uppercase"><tr><th class="p-4">التاريخ</th><th class="p-4">الحالة</th><th class="p-4">الحفظ</th><th class="p-4 text-center">التقييم</th></tr></thead>
                            <tbody class="divide-y divide-gray-50">
                                ${logs.map(l => `<tr><td class="p-4">${l.date}</td><td class="p-4">${l.attendance}</td><td class="p-4">${l.hifz || '--'}</td><td class="p-4 text-center">${l.exam || '--'}</td></tr>`).join('')}
                            </tbody>
                        </table>
                    </div>
                `;
            ...
            calculateAttendanceRate(studentId) {
                ...
                return Math.round((logs.filter(l => l.attendance === 'حاضر').length / logs.length) * 100);
            },

            renderHonorRoll() {
                const topStudents = [...this.db.students].filter(s => s.status === 'نشط').sort((a, b) => (b.points || 0) - (a.points || 0)).slice(0, 3);
                if (topStudents.length === 0) return '';
                return `
                    <div class="bg-gradient-to-br from-yellow-50 to-white p-6 rounded-3xl border border-yellow-200 mb-8">
                        <h3 class="font-black text-orange-800 mb-4 flex items-center text-lg"><i class="fas fa-crown ml-3 text-yellow-500"></i> نجوم الفرقان</h3>
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            ${topStudents.map((s, idx) => `<div class="bg-white p-4 rounded-2xl flex items-center space-x-4 space-x-reverse border shadow-sm">
                                <div class="text-2xl">${idx === 0 ? '🥇' : idx === 1 ? '🥈' : '🥉'}</div>
                                <div><p class="font-black text-sm text-gray-800">${s.name}</p><p class="text-[10px] text-orange-600">${s.points || 0} نقطة</p></div>
                            </div>`).join('')}
                        </div>
                    </div>
                `;
            ...
            checkAbsenteeism(studentId) {
                ...
                return logs.slice(0, 3).every(l => l.attendance === 'غائب');
            },
            ...
            openChangePasswordModal() {
                ...
                if (newPass && newPass.length >= 3) {
                    ...
                    this.saveDB();
                    this.showToast("تم التغيير بنجاح", "success");
                }
            },
            ...
            printFinanceReport() {
                ...
                win.document.write(`<dir dir="rtl" style="font-family: 'Cairo', sans-serif; padding: 40px;">
                    <h1 style="color: #1a5fb4;">مركز الفرقان - كشف حساب مالي</h1>
                    <table border="1" style="width: 100%; border-collapse: collapse; margin-top: 20px; text-align: right;">
                        <thead style="background: #eee;"><tr><th style="padding:10px">التاريخ</th><th>البيان</th><th>النوع</th><th>المبلغ</th></tr></thead>
                        <tbody>${f.transactions.map(t => `<tr><td style="padding:10px">${t.date.split('T')[0]}</td><td>${t.desc}</td><td>${t.type=='income'?'إيراد':'مصروف'}</td><td>${t.amount} ر.س</td></tr>`).join('')}</tbody>
                    </table></dir>`);
                win.document.close(); win.focus(); setTimeout(() => win.print(), 500);
            },

            renderSystemHealth() {
                const total = this.db.students.length;
                const active = this.db.students.filter(s => s.status === 'نشط').length;
                const healthPercent = total > 0 ? Math.round((active / total) * 100) : 0;
                return `<div class="bg-white p-4 rounded-2xl border mt-4 no-print"><div class="flex justify-between mb-2"><span class="text-[10px] font-black">نشاط الطلاب</span><span class="text-[10px] font-black">${healthPercent}%</span></div><div class="progress-bar"><div class="progress-fill" style="width: ${healthPercent}%"></div></div></div>`;
            },

            printStudentCertificate(studentId) {
                const s = this.db.students.find(x => x.id == studentId);
                let win = window.open('', '_blank');
                win.document.write(`<dir dir="rtl" style="font-family: 'Cairo', sans-serif; text-align: center; border: 10px double #1a5fb4; padding: 50px; margin: 20px;">
                    <h1 style="color: #1a5fb4;">شهادة تقدير</h1><p>يسر المركز أن يمنح الطالب:</p><h2>${s.name}</h2>
                    <p>لإتمامه حفظ (${s.juzCount}) جزءاً بمجموع نقاط تميز (${s.points || 0})</p></dir>`);
            ...
            handleGlobalSearch(query) {
                ...
                if (matches.length > 0) {
                    resDiv.innerHTML = matches.map(s => `<div onclick="App.navigate('admin-students'); App.renderAdminStudents('${s.name}'); document.getElementById('search-results').classList.add('hidden');" class="p-3 hover:bg-gray-50 cursor-pointer text-xs font-black border-b">${s.name}</div>`).join('');
                    resDiv.classList.remove('hidden');
                } else {
                    resDiv.innerHTML = '<p class="p-2 text-[10px] text-center">لا توجد نتائج</p>';
                    resDiv.classList.remove('hidden');
                }
            },
            ...
            sendWhatsApp(studentId) {
                const s = this.db.students.find(x => x.id == studentId);
                const l = this.db.dailyLogs.filter(d => d.studentId == studentId).sort((a,b) => new Date(b.date) - new Date(a.date))[0];
                if(!l) return this.showToast("لا يوجد تقرير اليوم", "error");
                const msg = `تقرير الطالب: ${s.name}%0Aالحالة: ${l.attendance}%0Aالحفظ: ${l.hifz || '--'}%0Aالمراجعة: ${l.murajaa || '--'}%0Aالتقييم: ${l.exam || '--'}`;
                window.open(`https://wa.me/972${s.phone.replace(/^0+/, '')}?text=${msg}`, '_blank');
            },
            ...
            setupTestForm() {
                ...
                form.onsubmit = (e) => {
                    e.preventDefault();
                    const sid = document.getElementById('test-student-id').value;
                    const test = { id: Date.now(), studentId: sid, part: document.getElementById('test-part').value, score: document.getElementById('test-score').value, grade: document.getElementById('test-grade').value, date: new Date().toISOString().split('T')[0] };
                    if(!this.db.tests) this.db.tests = [];
                    this.db.tests.push(test);
                    const sIdx = this.db.students.findIndex(s => s.id == sid);
                    if(sIdx > -1) this.db.students[sIdx].points = (this.db.students[sIdx].points || 0) + 50;
                    this.saveDB(); this.closeModal('modal-add-test'); this.showToast("تم تسجيل الاختبار ومنح 50 نقطة", "success"); form.reset();
                };
            }
        };
